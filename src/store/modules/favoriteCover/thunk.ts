...
// { id, company, coverPrice, totalPages, persons, title, cover, year, description }
...
export const RemFavThunk = (value: { id: number; company: string; coverPrice: string; totalPages: number; persons: object[]; title: string; cover: string; year: number; description: string }) => (dispatch: (AddFav: { value: { id: number; company: string; coverPrice: string; totalPages: number; persons: object[]; title: string; cover: string; year: number; description: string } }) => any, setState: () => any) => {
    localStorage.setItem("@CONAN/FavoriteList", JSON.stringify(JSON.parse(localStorage.getItem("@CONAN/FavoriteList") || "[]").filter((elt: string[]) => JSON.stringify(elt) !== JSON.stringify(value))))
    dispatch(AddFav(value))
}
